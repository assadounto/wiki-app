...
import { getDetails2 } from "../redux/details";
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faChevronLeft,faGear,faMicrophone,faSpinner } from '@fortawesome/free-solid-svg-icons'
...
const Country =() =>{
  const { country,lat,lon } = useParams();
    ...
    return(
    <div>
    {  status.isCountryinfoFailed
          ? <div className="load">Loading Faild</div>
          : 
        ...
        {status.items2.map((key) => (
            <div className='country-info' key={key.name}>
             ...
             <div className="img"><img src={key.flag} alt='' /></div>
              ...
              <div>Population : {key.population}</div>
             </div>
             </div>
        ...
        {status.items.map((key) => (
            <div className='Pollution-info' key={key.nitro} >
              <div className="Quality">Air Quality {key.AirQuality===1?<div className="G C">Good</div>:2?<div className="G C">fair</div>:3?<div className="B C">moderate</div>:4?<div className="R C">poor</div>:<div className="R C">very poor</div>}<div>Details Below</div></div>
  ...
  <div>Air Quality Index is {key.AirQuality}</div>
